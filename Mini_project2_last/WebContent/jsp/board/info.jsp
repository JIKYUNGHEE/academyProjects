...
<body>
	...
	<div class="board-card-list">
		<ul>
			<c:forEach var="fInfo" items="${ requestScope.fInfo }">
				<c:forEach var="info" items="${ boardPage.boardList }">
					<c:if test="${ fInfo.boardNo eq info.no }">
						<li><a
							href="<%= request.getContextPath() %>/infoDetail.do?no=${ fInfo.boardNo }">
								<span class="card-img"> <img alt=""
									src="<%= request.getContextPath() %>/upload/${ fInfo.saveName }">
							</span> <strong class="card-tit">${ info.title }</strong>
						</a></li>
					</c:if>
				</c:forEach>
			</c:forEach>
	...
	<c:if test="${boardPage.startPage > 5}">
			<form class="pagingBox" action="<%= request.getContextPath() %>/info.do" method="post">
